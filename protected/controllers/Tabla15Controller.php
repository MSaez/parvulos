...
class Tabla15Controller extends Controller
{
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Tabla15']))
		{
			$model->attributes=$_POST['Tabla15'];
			...
			if(($model->A1_E1=='L') && ($model->A2_E1=='L') && ($model->A3_E1=='L') && ($model->A4_E1=='L') && ($model->A5_E1=='L'))
			{
				$L1_1=1;
			}
			else
			...
			// determinacion de logro NT1 para evaluacion 1
			if(($model->A6_E1=='L') && ($model->A7_E1=='L') && ($model->A8_E1=='L') && ($model->A9_E1=='L') && ($model->A10_E1=='L'))
			{
				$L2_1=1;
			}
			else
			...
			if(($model->A11_E1=='L') && ($model->A12_E1=='L') && ($model->A13_E1=='L') && ($model->A14_E1=='L') && ($model->A15_E1=='L'))
			{
				$L3_1=1;
			}
			else
			...
			if(($model->A16_E1=='L') && ($model->A17_E1=='L') && ($model->A18_E1=='L') && ($model->A19_E1=='L') && ($model->A20_E1=='L'))
			{
				$L4_1=1;
			}
			else
			...
			if(($model->A1_E2=='L') && ($model->A2_E2=='L') && ($model->A3_E2=='L') && ($model->A4_E2=='L') && ($model->A5_E2=='L'))
			{
				$L1_2=1;
			}
			else
			...
			// determinacion de logro NT1 para evaluacion 2
			if(($model->A6_E2=='L') && ($model->A7_E2=='L') && ($model->A8_E2=='L') && ($model->A9_E2=='L') && ($model->A10_E2=='L'))
			{
				$L2_2=1;
			}
			else
			...
			if(($model->A11_E2=='L') && ($model->A12_E2=='L') && ($model->A13_E2=='L') && ($model->A14_E2=='L') && ($model->A15_E2=='L'))
			{
				$L3_2=1;
			}
			else
			...
			if(($model->A16_E2=='L') && ($model->A17_E2=='L') && ($model->A18_E2=='L') && ($model->A19_E2=='L') && ($model->A20_E2=='L'))
			{
				$L4_2=1;
			}
			else
			...
			if(($model->A1_E3=='L') && ($model->A2_E3=='L') && ($model->A3_E3=='L') && ($model->A4_E3=='L') && ($model->A5_E3=='L'))
			{
				$L1_3=1;
			}
			else
			...
			// determinacion de logro NT1 para evaluacion 3
			if(($model->A6_E3=='L') && ($model->A7_E3=='L') && ($model->A8_E3=='L') && ($model->A9_E3=='L') && ($model->A10_E3=='L'))
			{
				$L2_3=1;
			}
			else
			...
			if(($model->A11_E3=='L') && ($model->A12_E3=='L') && ($model->A3_E13=='L') && ($model->A14_E3=='L') && ($model->A15_E3=='L'))
			{
				$L3_3=1;
			}
			else
			...
			if(($model->A16_E3=='L') && ($model->A17_E3=='L') && ($model->A18_E3=='L') && ($model->A19_E3=='L') && ($model->A20_E3=='L'))
			{
				$L4_3=1;
			}
			else
			...
			$model ->E2 = $lv2;
			$model ->E3 = $lv3;
			$model ->ANIO_C = $cursa->ANIO_C;  // tomamos el año de la tabla cursa
			if ($alumno != null)
				$model->RUT_A = $alumno->RUT_A;
			
			if($model->save())
				$this->redirect(array('view','id'=>$model->ID_T15));
		}
	...
	{
		$dataProvider=new CActiveDataProvider('Tabla15');
	...
	{
		$model=new Tabla15('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Tabla15']))
			$model->attributes=$_GET['Tabla15'];
	 ...
	 * @return Tabla15 the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Tabla15::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Tabla15 $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='tabla15-form')
		{
